...
class Semester{
     ...
     */
    function createSemester($semester_number,$branch_id,$created_by,$updated_by){

        $rows = array("semester_number","branch_id","created_by","updated_by");
        $values = array($this->convertToSTring($semester_number),$this->convertToSTring($branch_id),$this->convertToSTring($created_by),$this->convertToSTring($updated_by));
        $pdo = $this->includes();
        $pdo->insertDb("semester",$rows,$values);
    }
    ...
    function updateSemester($semester_number,$last_passed_semester,$branch_id,$updated_by,$condition){
        ...
        $pdo = $this->includes();
        $pdo->updateDb("semester",$field,$condition);
    ...
    function deleteSemester($semester_id){
        $pdo = $this->includes();
        $pdo->updateDb("semester","deleted=1","semester_id=$semester_id");
    }
