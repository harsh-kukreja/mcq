        ...
        <div class="table-responsive" >
            <table class="table table-hover" >
                ...
                <?php
                    $pdoObject = new PdoConnection();
                    $pdo = $pdoObject->connectPdo();
                    $statement = $pdo->prepare($query="SELECT CONCAT(person.first_name, CONCAT(\" \", person.last_name)) as name, person.email, person.address, person.contact, batch.batch_name, division.division_name, branch.branch_name, branch.branch_code, semester.semester_number ,student.student_id FROM student INNER JOIN user ON student.user_id = user.user_id INNER JOIN person ON user.person_id = person.person_id INNER JOIN batch ON student.batch_id = batch.batch_id INNER JOIN division ON batch.division_id = division.division_id INNER JOIN branch ON division.branch_id = branch.branch_id INNER JOIN semester ON student.semester_id = semester.semester_id ");
//                    $parent_statement = $pdo->prepare($query="SELECT CONCAT(person.first_name, CONCAT(\" \", person.last_name)) as parent_name FROM parent_student INNER JOIN parent ON parent.parent_id = parent_student.parent_id INNER JOIN user ON user.user_id = parent.user_id INNER JOIN person ON person.person_id = user.person_id WHERE parent_student.student_id");
